...
class Repeat:
    ...
    def Reflask(self):
        global data,it_json, corona_notice_json, corona_new_json
        data = loadJson_assign()
        it_json = loadJson_it()
...
def assign():
    ...
    if content == u"전부":
        text = ""
        assign_list = list(data.keys())
        for i in range(len(data)):
            text = text + "< " + assign_list[i] + " 과제" + " >\n\n"
            if len(data[assign_list[i]])==0:
                text = text + "과제가 없습니다.\n\n"
            for j in data[assign_list[i]]:
                text = text + "과제명 : " + j['title'] + "\n제출기한 : " + j['date'] + "\n\n"
    ...
    elif content in list(data.keys()):
        text = "< " + content + " 과제" + " >\n\n"
        if len(data[content]) == 0:
            text = text + "과제가 없습니다.\n\n"
        for i in data[content]:
            text = text + "과제명 : " + i['title'] + "\n제출기한 : " + i['date'] + "\n\n"
        response_data = {
                "version" : "2.0",
                "template" : {
                    "outputs" : [
